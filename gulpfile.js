...
var paths = {
  dir: {
    app: './app', // Путь к development-папке (path to development folder)
    dist: './dist' // Путь к production-папке (path to production folder)
  },
  watch: {
    html: './app/pug/**/*.pug', // Путь для вотчера Pug-файлов (path for watcher to Pug files)
    css: './app/sass/**/*.scss' // Путь для вотчера Sass-файлов (path for watcher to Sass files)
  },
  app: {
    html: { // Пути для таска html (paths for html task)
      src: './app/pug/email.pug', // Путь к Pug-файлам для таска html (path for html task to Pug files)
      dest: './app' // Место сохранения html-шаблона письма (place of saving html template of e-mail)
    },
    css: { // Пути для таска css (paths for css task)
      ...
      dest: './app/css' // Место сохранения CSS-файлов (place of saving CSS files)
    }
  },
  dist: { // Пути для production (paths for production)
    src: './app/email.html', // Исходный HTML-шаблон письма из development-папки (source html template of e-mail from development folder app)
    dest: './dist' // Место сохранения HTML-шаблона с встроенными инлайн-стилями для production (place of saving HTML template with inline styles for production)
  }
}
